...
export default function ImagePlaceholder({ label = 'Image', ratio = 'aspect-[4/3]', src, className = '' }) {
  if (src) {
    return <img src={src} alt={label} className={`w-full h-full object-cover ${ratio} ${className}`} />
  }
  return (
    <div
      className={`relative ${ratio} ${className} grain-bg flex flex-col items-center justify-center gap-2 border border-gold-line bg-charcoal2 overflow-hidden group`}
    >
      ...
      >
        ...
        <span className="text-[10px] text-smoke/60">Replace in /public</span>
      </motion.div>
